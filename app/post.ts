...
export type PostMarkdownAttribute = {
  title: string;
};
...
function isValidPostAttributes(
  attributes: any
): attributes is PostMarkdownAttribute {
...
export async function editPost(post: EditPost) {
  const currentPath = path.join(postsPath, post.currentSlug + ".md");

  invariant(fs.access(currentPath), "Unable to edit post!");
